...
//  printf("mac header parser\n");
//  printf("mac header: %02x %02x ; %u \n", *p, *(p+1),fc );
//	printf("path type == %d \n", path_type);
// printf("macheader: pkt_len=%d \n", pkt_len);
...
//		printf("beacon \n");
    ...
    mlh.seq_ctrl =  pletohs(&(mgmt_h->seq_ctrl)); //EXTRACT_LE_16BITS(mgmt_h->seq_ctrl);  //Copied the common portions 
	//		printf("before becaon parse \n");
      ...
      //TODO :XXX where is the mac address print_mac(paket->mac_a,"beacon" );
      break;
			}
    default :
//		printf("defaul type \n");
			//TODO:XXX:FIXME : open the code path
...
int mac_header_err_parser(unsigned char *p,
 ...
 // printf("in mac header err parser \n"); 
  struct mgmt_header_t *mgmt_h =NULL;
 // printf("macheader: pkt_len=%d \n", pkt_len);
  memset(&clh,0,sizeof(struct control_layer_header));
  memset(&mlh_t,0,sizeof(struct mgmt_layer_err_header));
...
#undef ADDR3
  ...
  default :
	;
    // TODO: XXX classify it as different kinds of packets depending on packet length     
    /*
...
/*FUNCTIONS FOR PARSING MGMT FRAMES */
int parse_elements(struct mgmt_body_t *pbody, const u_char *p, int offset,u_int length, struct mgmt_beacon_layer_header * mlh)
{
  struct ssid_t ssid;
