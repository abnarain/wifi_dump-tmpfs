...
int rx_path(const unsigned char * p, int pkt_len, int cap_len )
{
  ...
  if (present & BIT(IEEE80211_RADIOTAP_RX_FLAGS )){
    ...
    if (rx_flags & 0x20 /*IEEE80211_RADIOTAP_F_HOMESAW_FAILED_PHY*/){
      /* This is removed as it will generate an order more of traffic with
      totally garbage frame which were received by hardware and even the 
      mac header was not decoded properly
      */
...
int drops(void)
{
  ...
  if (ps_drop >0){
    ...
    snprintf(update_drops_filename,FILENAME_MAX,UPDATE_DROPS_FILENAME, \
...
static void initialize_bismark_id(void)
{
