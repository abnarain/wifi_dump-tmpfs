...
u_int8_t* access_point_address_table_lookup(mac_address_table_t*  table,
			  uint8_t* mac,int mgmt_type){
  if (table->length > 0) {
...
int write_update(){
 ...
 mgmt_beacon_count =0; 
  ...
  snprintf(update_data_filename,FILENAME_MAX,UPDATE_DATA_FILENAME,bismark_id,start_timestamp_microseconds,sequence_number,ifc);
  ...
  snprintf(update_control_filename,FILENAME_MAX,UPDATE_CONTROL_FILENAME,bismark_id,start_timestamp_microseconds,sequence_number,ifc);
